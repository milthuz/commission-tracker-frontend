...
// Sofia — the Sales Hub guide. Uses the Sales Hub Monogram-S glyph (white arc + orange node) on a
// navy disc (#1c2434), matching the sidebar. Inline SVG so it scales crisply with no asset load.
// Size via className (h-/w-).
const SofiaAvatar: React.FC<{ className?: string; ring?: boolean }> = ({ className = 'h-9 w-9', ring = false }) => (
  <span className={`relative inline-flex items-center justify-center overflow-hidden rounded-full ${ring ? 'ring-2 ring-white/70' : ''} ${className}`}>
    <svg viewBox="0 0 64 64" className="h-full w-full" role="img" aria-label="Sofia">
