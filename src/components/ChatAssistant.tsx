...
const ChatAssistant: React.FC = () => {
  ...
  return (
    <>
      {/* Chat panel */}
      {open && (
        <div className="fixed bottom-24 right-4 z-[9990] flex h-[min(560px,calc(100vh-8rem))] w-[min(380px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-white/40 bg-white/90 shadow-2xl backdrop-blur-xl dark:border-strokedark/60 dark:bg-boxdark/90">
          {/* Header */}
          ...
          <div ref={bodyRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(suggestions) && suggestions.map((s) => (
                    <button key={s} onClick={() => send(s)}
                      className="rounded-full border border-stroke bg-white/70 px-3 py-1.5 text-left text-xs text-body backdrop-blur transition hover:border-primary hover:text-primary dark:border-strokedark dark:bg-meta-4/60 dark:text-bodydark">
                      {s}
                    </button>
                  ))}
                </div>
            ...
            {msgs.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                  m.role === 'user'
                    ? 'rounded-br-sm bg-[#1c2434] text-white dark:bg-primary'
                    : 'rounded-tl-sm bg-gray-100 text-black dark:bg-meta-4 dark:text-white'
                }`}>
                  {m.content}
                </div>
              </div>
            ...
            {busy && (
              <div className="flex justify-start">
                <div className="flex items-center gap-1.5 rounded-2xl rounded-tl-sm bg-gray-100 px-4 py-3 dark:bg-meta-4">
          ...
          <form
            ...
            className="flex items-center gap-2 border-t border-stroke bg-white/60 px-3 py-3 backdrop-blur dark:border-strokedark dark:bg-boxdark/60"
          >
            <input
              ref={inputRef}
              value={input}
              ...
              maxLength={2000}
              className="flex-1 rounded-full border border-stroke bg-white px-4 py-2.5 text-sm text-black outline-none transition focus:border-primary dark:border-strokedark dark:bg-meta-4 dark:text-white"
            />
          ...
          <p className="bg-white/60 px-4 pb-2 text-center text-[10px] text-body backdrop-blur dark:bg-boxdark/60">
