...
// Simplified analog of Header/DropdownUser.tsx, scoped to PartnerAuthContext — no Version
// History/Support links (those are internal-app-only), just profile + logout.
const PartnerDropdownUser = () => {
    ...
    <div className="relative">
      <Link ref={trigger} onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-4" to="#">
        <span className="hidden text-right lg:block">
          <span className="block text-sm font-medium text-black dark:text-white">{user?.name || 'User'}</span>
...
export default PartnerDropdownUser;
