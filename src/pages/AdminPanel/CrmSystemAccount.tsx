...
// Le compte Zoho sous lequel l'application ECRIT.
//
...
//
// La sonde n'est pas décorative. Un jeton peut exister, se rafraîchir, et ne rien pouvoir
// faire — c'est arrivé le 2026-09-03 avec un profil Zoho sans accès API, qui répondait 403 à
// tout. Épingler un compte pareil casserait toutes les écritures en silence, alors le serveur
// refuse d'épingler ce qu'il n'a pas réussi à lire.
...
type Sonde = {
  ...
  zohoUser?: { name: string | null; email: string | null; profile: string | null; role: string | null; admin: boolean };
  modules?: number;
};

export default function CrmSystemAccount() {
  ...
  const charger = async (probe?: string) => {
    try {
      const q = probe ? `?probe=${encodeURIComponent(probe)}` : '';
      const r = await fetch(`${API_URL}/api/admin/crm-system-account${q}`, { headers: authHeaders() });
      if (!r.ok) return;
  ...
  const epingler = async (email: string) => {
    ...
    try {
      const r = await fetch(`${API_URL}/api/admin/crm-system-account`, {
    ...
    <div className="mt-6 rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="p-7">
        ...
        {/* Le résultat de la sonde, en clair. C'est le seul endroit où l'on voit si un jeton
            peut réellement faire quelque chose — un profil Zoho sans accès API ressemble en
            tout point à un profil valide tant qu'on ne lui demande rien. */}
        {sonde && (
        ...
        {/* Brancher un compte de service : il n'a pas de session Sales Hub, donc il ne peut pas
            venir cliquer lui-même. On déclare l'adresse, et on signe chez Zoho en tant que lui. */}
